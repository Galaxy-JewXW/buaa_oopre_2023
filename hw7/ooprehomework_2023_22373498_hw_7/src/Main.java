...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if ((op = Integer.parseInt(l.get(0))) != 15) {
                int advId = Integer.parseInt(l.get(1));
                if (op == 1) { //加入一个需要管理的冒险者（新加入的冒险者不携带任何药水瓶和装备）
                    mainMap.put(advId, new Adventurer(advId, l.get(2)));
                } else if (op == 2) { //给某个冒险者增加一个药水瓶
                    createObj(l, true);
                } else if (op == 3) { //删除某个冒险者的某个药水瓶
                    shop.buyB(mainMap.get(advId).deleteBottle(Integer.parseInt(l.get(2))));
                } else if (op == 4) { //给某个冒险者增加一个装备
                    createObj(l, false);
                } else if (op == 5) { //删除某个冒险者的某个装备
                    shop.buyE(mainMap.get(advId).deleteEquipment(Integer.parseInt(l.get(2))));
                    ...
                    shop.buyF(mainMap.get(advId).deleteFood(Integer.parseInt(l.get(2))));
                    ...
                    mainMap.get(advId).sellAll(shop);
                } else if (op == 23) {
                    shop.sell(mainMap.get(advId), l);
                }
            } else {
