...
public class Bag {
    ...
    public int useBottle(int bottleId, int hitPoint) {
        if (bottleBag.containsKey(bottleId)) {
            int res = 0;
            Bottle bottle = bottleBag.get(bottleId);
            switch (bottle.getType()) {
                case 1: {
                    res = bottle.getCapacity();
                    break;
                } case 2: {
                    res = (int) Math.floor((1 + bottle.getRatio()) * bottle.getCapacity());
                    break;
                } case 3: {
                    res = (int) Math.floor(bottle.getRatio() * hitPoint);
                    break;
                } default:
                    break;
            }
            if (!bottle.getStatus()) { // used
                return 0;
            } else {
                bottle.setStatus(false);
                return res;
            }
        } else {
            return -1;
