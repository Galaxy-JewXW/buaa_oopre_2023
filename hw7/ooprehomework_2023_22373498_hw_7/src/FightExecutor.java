...
public class FightExecutor {
    ...
    public void exchangeMoney() {
        for (Map.Entry<Integer, Adventurer> adventurerEntry : participants.entrySet()) {
            Adventurer a = adventurerEntry.getValue();
            if ((a.getHitPoint() + a.getLoss()) / 2 < a.getHitPoint()) {
                continue;
            }
            for (Map.Entry<Integer, Adventurer> entry : a.getHiredMap().entrySet()) {
                Adventurer b = entry.getValue();
                long money = a.getLoss() * 10000L;
                money = Math.min(money, b.getMoney());
                a.addMoney(money);
                b.useMoney(money);
            }
        }
